...
# Asserts must be compiled OUT of EVERY shipped Mesa dylib (2026-08-07). Mesa's common
# Vulkan runtime and KK together carry ~820 assert()s, and zink adds its own on state a
# guest's GL stream drives; an assert anywhere in that stack SIGABRTs the worker, so a guest
# app's own bug kills the whole VM. Five such incidents so far (see the
# limina-kk-empty-clear-rect memory) — the fifth was zink's
# `assert(!pStencilAttachment || stencilAttachmentFormat)` in begin_rendering, which is why
# this covers the gallium/EGL roots and not just KK: guarding one dylib of a stack proves
# nothing about the others, and the one left unguarded is the one that shipped asserts.
# The build option is `-Db_ndebug=true` on EVERY build dir (keep buildtype=debugoptimized:
# -O2 and -g stay, so crash reports still symbolicate). This is a TRIPWIRE, not the fix —
# invalid guest input is rejected at the vkr trust boundary; asserts-off is the backstop for
# the ones we have not found yet. It lives here because the option is meson state on the
# sparse image, NOT anything this repo pins: a fresh `meson setup` silently restores asserts,
# and nothing else would notice.
for f in "${DLOPEN_ROOTS[@]}"; do
  nm -u "$f" 2>/dev/null | grep -q "assert_rtn" || continue
  echo "REFUSING to bundle: $(basename "$f") still references __assert_rtn — it was built" >&2
  echo "with asserts ON, so any guest can abort the worker and take the VM down. Rebuild the" >&2
  echo "build dir that produced it with asserts off, e.g.:" >&2
  echo "  meson configure /Volumes/mesa-cs/build-kk -Db_ndebug=true && ninja -C /Volumes/mesa-cs/build-kk" >&2
  echo "  meson configure /Volumes/mesa-cs/build-zink-kk -Db_ndebug=true &&" >&2
  echo "    ninja -C /Volumes/mesa-cs/build-zink-kk && meson install -C /Volumes/mesa-cs/build-zink-kk" >&2
  exit 1
done

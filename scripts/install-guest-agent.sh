...
ssh "${SSH_OPTS[@]}" "$HOST" '
    ...
    # The flat-pointer gschema override is RETIRED (2026-07-23): captured motion drives the
    # absolute tablet now, which libinput never accelerates. Clean it off guests that have it.
    if [ -f /usr/share/glib-2.0/schemas/90-limina-pointer.gschema.override ]; then
        sudo rm -f /usr/share/glib-2.0/schemas/90-limina-pointer.gschema.override
        sudo glib-compile-schemas /usr/share/glib-2.0/schemas/
        echo "removed retired 90-limina-pointer.gschema.override"
    fi
'

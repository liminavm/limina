...
# One leg of the 2026-09-23 async-decode pass: build limina's HEAD against a given virglrs +
...
#
# READING THIS PASS: the change moves hardware video decode off the control thread and puts a
# settle barrier on every vrend resource lookup, draw, dispatch, clear, transfer and fence. No arm
# decodes video, so what this pass can see is the barrier's cost when nothing is in flight -- one
# atomic load per lookup. The arms that enter vrend are the AQUARIUM (enhanced guest) and BASEMARK
# (stock guest); the zink->venus arms are a collateral-damage check.
#
# Usage: perf/async-decode-2026-09-23/point.sh <label> <virglrs-rev> <libkrun-rev>
...
DIR=perf/async-decode-2026-09-23
EV="$DIR/evidence/$LABEL"; LEDGER="$DIR/ledger.csv"
mkdir -p "$EV" perf/async-decode-2026-09-23/work.noindex/harness
...
ENH=perf/async-decode-2026-09-23/work.noindex/trend-enh.raw
STOCK=perf/async-decode-2026-09-23/work.noindex/trend-stock.raw
drop_clones() { rm -f perf/async-decode-2026-09-23/work.noindex/trend-enh.raw perf/async-decode-2026-09-23/work.noindex/trend-stock.raw; }
...
log "enhanced boot"
boot "$ENH" Fedora-Workstation-44.enhanced.raw "$ROOT/perf/async-decode-2026-09-23/work.noindex/aq-capture.png" ||
...
[ "${ARMS:-all}" = contended ] || for r in r1 r2; do
  LIMINA_SSH_PORT="$PORT" LIMINA_CAPTURE="$ROOT/perf/async-decode-2026-09-23/work.noindex/aq-capture.png" AQ_OUT="$EV/aquarium-$r" \
...
for f in client-basemark.sh marionette.py tap-keys.py; do   # one frozen harness for every point
  git -C third_party/virglrs show "d0416c9:harness/vm/$f" > "perf/async-decode-2026-09-23/work.noindex/harness/$f"
done
scp -P "$PORT" "${SSHO[@]}" -q perf/async-decode-2026-09-23/work.noindex/harness/client-basemark.sh perf/async-decode-2026-09-23/work.noindex/harness/marionette.py \
  perf/async-decode-2026-09-23/work.noindex/harness/tap-keys.py claude@127.0.0.1:/tmp/

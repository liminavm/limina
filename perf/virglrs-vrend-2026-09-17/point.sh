...
# One leg of the 2026-09-17 virglrs vrend pass (af70f7c..9041fa2): build limina's HEAD against a
# given virglrs + libkrun pair, then run the battery on it -- enhanced guest (aquarium 25k/30k x2,
# perf-ledger x3, vkmark x3 idle, vkmark x2 under a 25k aquarium) and stock guest (Basemark
# Graphics suite, second run scored). A copy of perf/virglrs-robustness-2026-09-17/point.sh with
# this pass's directory; rows go to this directory's ledger, never to perf/ledger.csv. Aquarium fps
# is only drawn on screen: read the *-fps.png crops afterwards and add those rows by hand.
#
# READING THIS PASS: every commit under test is vrend. The enhanced guest's perf-ledger and vkmark
# arms run zink->venus and never enter vrend, so they are a collateral-damage check and cannot
# score the change. The arms that do enter vrend are the AQUARIUM (enhanced guest) and BASEMARK
# (stock guest). A flat venus arm here is arithmetic, not evidence.
#
# Usage: perf/virglrs-vrend-2026-09-17/point.sh <label> <virglrs-rev> <libkrun-rev>
...
DIR=perf/virglrs-vrend-2026-09-17
EV="$DIR/evidence/$LABEL"; LEDGER="$DIR/ledger.csv"
mkdir -p "$EV" perf/virglrs-vrend-2026-09-17/work.noindex/harness
...
ENH=perf/virglrs-vrend-2026-09-17/work.noindex/trend-enh.raw
STOCK=perf/virglrs-vrend-2026-09-17/work.noindex/trend-stock.raw
drop_clones() { rm -f perf/virglrs-vrend-2026-09-17/work.noindex/trend-enh.raw perf/virglrs-vrend-2026-09-17/work.noindex/trend-stock.raw; }
...
log "enhanced boot"
boot "$ENH" Fedora-Workstation-44.enhanced.raw "$ROOT/perf/virglrs-vrend-2026-09-17/work.noindex/aq-capture.png" ||
...
[ "${ARMS:-all}" = contended ] || for r in r1 r2; do
  LIMINA_SSH_PORT="$PORT" LIMINA_CAPTURE="$ROOT/perf/virglrs-vrend-2026-09-17/work.noindex/aq-capture.png" AQ_OUT="$EV/aquarium-$r" \
...
for f in client-basemark.sh marionette.py tap-keys.py; do   # one frozen harness for every point
  git -C third_party/virglrs show "d0416c9:harness/vm/$f" > "perf/virglrs-vrend-2026-09-17/work.noindex/harness/$f"
done
scp -P "$PORT" "${SSHO[@]}" -q perf/virglrs-vrend-2026-09-17/work.noindex/harness/client-basemark.sh perf/virglrs-vrend-2026-09-17/work.noindex/harness/marionette.py \
  perf/virglrs-vrend-2026-09-17/work.noindex/harness/tap-keys.py claude@127.0.0.1:/tmp/

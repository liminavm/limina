...
# One leg of the 2026-09-24 pin pass: build limina's HEAD against a given virglrs +
# libkrun pair, then run the battery on it -- enhanced guest (aquarium 25k/30k x2, perf-ledger x3,
# vkmark x3 idle, vkmark x2 under a 25k aquarium) and stock guest (Basemark Graphics suite, second
# run scored). A copy of perf/virglrs-vrend-2026-09-17/point.sh with this pass's directory; rows go
# to this directory's ledger, never to perf/ledger.csv. Aquarium fps is only drawn on screen: read
# the *-fps.png crops afterwards and add those rows by hand.
#
# READING THIS PASS: the pin moves virglrs b900066 -> 828a05b and libkrun 32cc3776 -> 26450d76.
# That carries a VideoToolbox warm-up at renderer start (one VP9 session built and dropped on a
# thread of its own), decode stats, and the virglrs session's work since: host-minted blob pages
# charged to their context, maintenance6, no multisampling advertised without multisample array
# storage, checked sampler/image slots, unsafe-handle construction in venus. No arm decodes video,
# so the warm-up shows only as startup; the venus and vrend arms are what the other work can move.
#
# Usage: perf/pin-2026-09-24/point.sh <label> <virglrs-rev> <libkrun-rev>
...
DIR=perf/pin-2026-09-24
EV="$DIR/evidence/$LABEL"; LEDGER="$DIR/ledger.csv"
mkdir -p "$EV" perf/pin-2026-09-24/work.noindex/harness
...
ENH=perf/pin-2026-09-24/work.noindex/trend-enh.raw
STOCK=perf/pin-2026-09-24/work.noindex/trend-stock.raw
drop_clones() { rm -f perf/pin-2026-09-24/work.noindex/trend-enh.raw perf/pin-2026-09-24/work.noindex/trend-stock.raw; }
...
log "enhanced boot"
boot "$ENH" Fedora-Workstation-44.enhanced.raw "$ROOT/perf/pin-2026-09-24/work.noindex/aq-capture.png" ||
...
if [ "${ARMS:-all}" = contended ]; then ENH_ONLY=1; LEDGER_RUNS="1"; else LEDGER_RUNS="1 2 3"; fi
[ "${ARMS:-all}" = contended ] || for r in r1 r2; do
  LIMINA_SSH_PORT="$PORT" LIMINA_CAPTURE="$ROOT/perf/pin-2026-09-24/work.noindex/aq-capture.png" AQ_OUT="$EV/aquarium-$r" \
...
for r in $LEDGER_RUNS; do
  LIMINA_SSH_PORT="$PORT" LIMINA_PERF_LEDGER="$LEDGER" LIMINA_PERF_RATE=60 step 900 scripts/perf-ledger.sh "$NOTE; run $r/3 $(valid)" \
...
[ "${ARMS:-all}" = contended ] || for r in 1 2 3; do
...
# vkmark under a 25k aquarium: the contended arm (perf/2026-09-09-remeasure.md). NOTE 2026-09-16:
# this arm clusters per BOOT (~1300 or ~1600, tracking the aquarium fps beside it), so as run it
# cannot score a contention change -- it is kept for continuity, not for attribution.
"${SSH[@]}" "export XDG_RUNTIME_DIR=/run/user/1000 DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/1000/bus
  systemctl --user stop ff-bench 2>/dev/null || true; systemctl --user reset-failed ff-bench 2>/dev/null || true; sleep 3
  ...
  systemd-run --user --unit=ff-bench --setenv=WAYLAND_DISPLAY=wayland-0 --setenv=MOZ_ENABLE_WAYLAND=1 \
    ...
    /usr/bin/firefox --kiosk 'https://webglsamples.org/aquarium/aquarium.html?numFish=25000' >/dev/null"
sleep 35
# Capture-until-valid, as aquarium-run.sh does: the supervisor rewrites the capture in place, so a
# plain cp can read a torn or empty file, and a frame from before the page painted carries no
# counter. Require a new dump, then a crop with bright text; the rows say UNPROVEN otherwise, since
# a vkmark with no aquarium beside it is the idle number wearing the wrong name.
CAP="$ROOT/$DIR/work.noindex/aq-capture.png"; LOADPROOF="load UNPROVEN"
for attempt in $(seq 1 12); do
  before=$(stat -f %m "$CAP" 2>/dev/null || echo 0)
  for _ in $(seq 1 30); do [ "$(stat -f %m "$CAP" 2>/dev/null || echo 0)" -gt "$before" ] && break; sleep 1; done
  sleep 0.5; cp "$CAP" "$EV/contended-load.png"
  if scripts/perf/crop-fps.py "$EV/contended-load.png" "$EV/contended-load-fps.png" --require-content >/dev/null 2>&1; then
    LOADPROOF=""; break
  fi
  sleep 5
done
[ -z "$LOADPROOF" ] || log "WARNING: no aquarium counter in the contended-arm capture after 12 attempts"
for r in 1 2; do
  step 600 "${SSH[@]}" 'export XDG_RUNTIME_DIR=/run/user/1000 WAYLAND_DISPLAY=wayland-0; vkmark -s 1280x720' \
    > "$EV/vkmark-contended-run$r.txt" 2>&1
  s=$(sed -n 's/.*vkmark Score: \([0-9]*\).*/\1/p' "$EV/vkmark-contended-run$r.txt")
  log "vkmark under aquarium-25k run $r: ${s:-NO SCORE} $(valid)"
  [ -n "$s" ] && row vkmark-under-aquarium25k-venus score "$s" "run $r/2 $LOADPROOF $(valid)"
done
...
for f in client-basemark.sh marionette.py tap-keys.py; do   # one frozen harness for every point
  git -C third_party/virglrs show "d0416c9:harness/vm/$f" > "perf/pin-2026-09-24/work.noindex/harness/$f"
done
scp -P "$PORT" "${SSHO[@]}" -q perf/pin-2026-09-24/work.noindex/harness/client-basemark.sh perf/pin-2026-09-24/work.noindex/harness/marionette.py \
  perf/pin-2026-09-24/work.noindex/harness/tap-keys.py claude@127.0.0.1:/tmp/

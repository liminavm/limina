...
# Fifth round: narrow Basemark WebGL 2.0's step inside p05..p06 (c6b0d21..24a91eb). p05 read 4674
# on its one boot, p06 4271 and 3813, 6123e2e 4185-4260 on three. WebGL 2.0 spreads by hundreds of
# points on one tree, so every tree named here is run until it has three scored runs.
#
# Usage: perf/trend-d5889ef/bisect5.sh <rev>...   (short virglrs revs; attempts already in
...
# fa8c765..d1b6278 build against neither; run those with LIBKRUN_PATCH=perf/trend-d5889ef/libkrun-
...
point() { # <label> <rev> <libkrun>
  echo "######## $1 (libkrun ${3:0:8})"; STOCK_ONLY=1 perf/trend-d5889ef/point.sh "$1" "$2" "$3"
}

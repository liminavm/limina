...
# One leg of the 2026-09-16 virglrs pass: build limina's HEAD against a given virglrs + libkrun
# pair, then run the battery on it -- enhanced guest (aquarium 25k/30k x2, perf-ledger x3, vkmark
# x3 idle, vkmark x2 under a 25k aquarium) and stock guest (Basemark Graphics suite, second run
# scored). A copy of perf/trend-d5889ef/point.sh with the contended vkmark arm added; rows go to
# this directory's ledger, never to perf/ledger.csv. Aquarium fps is only drawn on screen: read the
# *-fps.png crops afterwards and add those rows by hand.
#
# Usage: perf/virglrs-legs-2026-09-16/point.sh <label> <virglrs-rev> <libkrun-rev>
...
DIR=perf/virglrs-legs-2026-09-16
EV="$DIR/evidence/$LABEL"; LEDGER="$DIR/ledger.csv"
mkdir -p "$EV" perf/virglrs-legs-2026-09-16/work.noindex/harness
...
ENH=perf/virglrs-legs-2026-09-16/work.noindex/trend-enh.raw
STOCK=perf/virglrs-legs-2026-09-16/work.noindex/trend-stock.raw
drop_clones() { rm -f perf/virglrs-legs-2026-09-16/work.noindex/trend-enh.raw perf/virglrs-legs-2026-09-16/work.noindex/trend-stock.raw; }
...
# WATCHDOG. Before virglrs 58aa6d6 a venus client can poison the compositor's vrend context: the
...
log "enhanced boot"
boot "$ENH" Fedora-Workstation-44.enhanced.raw "$ROOT/perf/virglrs-legs-2026-09-16/work.noindex/aq-capture.png" ||
  { log "ABORT: enhanced boot"; kill_by_disk "$ENH"; keep_log "$ENH" enh; exit 3; }
watch_start "$ENH"
settle enh
# Aquarium first: it is the vrend reading, and before virglrs 58aa6d6 the first venus client
...
[ "${ARMS:-all}" = contended ] || for r in r1 r2; do
  LIMINA_SSH_PORT="$PORT" LIMINA_CAPTURE="$ROOT/perf/virglrs-legs-2026-09-16/work.noindex/aq-capture.png" AQ_OUT="$EV/aquarium-$r" \
...
# vkmark under a 25k aquarium: the contended arm (perf/2026-09-09-remeasure.md), the one
# condition a venus locking change is expected to move. The aquarium is launched the way
# aquarium-run.sh launches it and a crop of the counter is kept as proof the load was present.
...
# plain cp can read a torn or empty file (it did, on l0), and a frame from before the page painted
# carries no counter. Require a new dump, then a crop with bright text; the rows say UNPROVEN
# otherwise, since a vkmark with no aquarium beside it is the idle number wearing the wrong name.
...
for f in client-basemark.sh marionette.py tap-keys.py; do   # one frozen harness for every point
  git -C third_party/virglrs show "d0416c9:harness/vm/$f" > "perf/virglrs-legs-2026-09-16/work.noindex/harness/$f"
done
scp -P "$PORT" "${SSHO[@]}" -q perf/virglrs-legs-2026-09-16/work.noindex/harness/client-basemark.sh perf/virglrs-legs-2026-09-16/work.noindex/harness/marionette.py \
  perf/virglrs-legs-2026-09-16/work.noindex/harness/tap-keys.py claude@127.0.0.1:/tmp/

...
General Public License, version 2, plus two additional permissions stated in
[`LICENSES/LicenseRef-limina-exception.txt`](LICENSES/LicenseRef-limina-exception.txt).
The *linking exception* explicitly allows combining limina with Apache-2.0-licensed
material and conveying the result. It exists because limina statically links
...
The *platform distribution grant* lets the maintainers named in [`AUTHORS`](AUTHORS)
convey limina through application stores whose terms the GPL alone would not
allow; it restricts no one and takes no right from any recipient.
...
Contributions are accepted under the same terms, including both permissions,
with a Developer Certificate of Origin sign-off; contributors keep their own
copyright. See [`CONTRIBUTING.md`](CONTRIBUTING.md).

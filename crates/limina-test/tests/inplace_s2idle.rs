...
fn stock_guest_survives_inplace_s2idle_with_correct_clock() {
    ...
    // Suspend from inside the guest, scheduled a beat ahead so this SSH command returns
    // before the network freezes (same recipe as the venus session gate).
    guest
        .ssh_exec("sudo systemd-run --on-active=2 systemctl suspend -i >/dev/null 2>&1; echo armed")
        .expect("arming the in-guest suspend");

    // s2idle entry oracle: SSH stops answering once virtio-net freezes. Require two
    // consecutive failures so one dropped connection on a loaded host can't fake it.

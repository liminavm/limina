...
///
/// Without it the fallback is the day, not the minute, and that is deliberate: this script
/// reruns whenever any watched tree's refs move (a `git fetch` in a vendored tree counts),
/// and cargo recompiles the crate only when a rerun's *output* differs. A minute-resolution
/// clock made every such rerun a fresh value and so a fresh compile+link of the largest
/// crate we own, for a build whose revisions had not moved at all.
fn build_stamp() {
...
/// tree to ask falls back to the pin: edk2, whose firmware `scripts/build-krun-efi.sh` builds
/// by cloning that very rev inside a container, so there the pin is what shipped.
///
/// A revision and nothing else — no branch. The hash is the identity; the branch it happened
/// to be reachable from is a detail of this host, and the row has to read to someone who has
/// never built limina.
///
/// One record per dependency, `;`-separated, fields `|`-separated: name, revision. Parsed
/// back by `about::deps`.
fn dep_stamp(root: &Path) -> String {
    ...
    for (name, tree) in DEPS {
        ...
        // Neither a checkout nor a pin: the manifest was reshaped out from under this list.
        // Say nothing rather than show a dependency with no revision.
        let Some(rev) = head.or_else(pin) else {
            continue;
        };
        records.push(format!("{}|{}", field(name), field(&rev)));
    }
    records.join(";")
}

/// One field of a stamp record. A hash carries neither separator, but a manifest `rev` is
/// whatever someone typed there — a tag, a hand-edit — and one containing a separator would
/// split its record in two and garble every row after it.

...
//! a dependency with no tree to ask falls back to the pin: edk2, whose firmware
//! `scripts/build-krun-efi.sh` builds by cloning that very rev inside a container, so there
//! the pin is what shipped.
//!
//! A row is a name and a revision and nothing more. Which branch a checkout sat on, and
//! whether a revision was observed or pinned, are vocabulary of how limina is built; the
//! person reading this dialog is filing a bug, and the hash is what answers them.
//!
...
pub(crate) struct Dep {
    ...
    /// The full commit the build carries. No branch beside it: the hash is the identity, and
    /// the branch it happened to be reachable from on the build host explains nothing to the
    /// person reading the bug report.
...
pub(crate) struct BuildInfo {
    ...
    /// When the binary was built, UTC: the minute a bundle was cut, or just the day for a
    /// dev build (`build.rs` explains why the clock there is deliberately coarse).
...
/// Parse the stamp: `;`-separated records of `name|rev`.
///
...
fn deps(stamp: &str) -> Vec<Dep> {
    stamp
        ...
        .filter_map(|record| {
            ...
            let (name, rev) = (fields.next()?, fields.next()?);
            if name.is_empty() || rev.is_empty() || fields.next().is_some() {
                return None;
            }
            Some(Dep {
                name: name.to_string(),
...
impl BuildInfo {
    ...
    pub(crate) fn plain_text(&self) -> String {
        ...
        for dep in &self.deps {
            // A name and a revision, nothing else. Everything a row could add — which branch
            // the checkout sat on, whether the revision was read from a tree or taken from
            // the manifest — is vocabulary of how limina is built, and this dialog is read by
            // someone filing a bug, not someone who builds it.
            out.push_str(&format!(
                "  {:width$}  {}\n",
                dep.name,
                dep.rev,
...
mod tests {
    ...
    /// A row is a name and a hash, in one aligned column, and carries no build vocabulary —
    /// no branch, no note on where the revision came from. Someone filing a bug reads this.
    #[test]
    fn a_row_is_a_name_and_a_revision() {
        let text = BuildInfo {
            version: "0.0.0",
            git_rev: "0000",
            built: "never",
            deps: deps("libkrun|aaaa;kosmickrisp|bbbb"),
        }
        .plain_text();
        assert!(text.contains("\n  libkrun      aaaa\n"), "rows:\n{text}");
        assert!(text.contains("\n  kosmickrisp  bbbb\n"), "rows:\n{text}");
        assert!(!text.contains("pin"), "build vocabulary leaked:\n{text}");
    }

    /// No row may outgrow the dialog's wrapping width: a wrapped row loses the column
    /// alignment the fixed-pitch font is there for.
    #[test]
    fn every_row_fits_the_dialogs_width() {
        let text = build_info().plain_text();
        // 11pt fixed pitch is ~6.6pt per character against setPreferredMaxLayoutWidth(620).
        let budget = (620.0_f64 / 6.6) as usize;

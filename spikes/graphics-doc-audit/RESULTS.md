...
| 8a | The **venus** present path (`SET_SCANOUT_BLOB` + MTLTexture import) works | `docs/graphics.md` §4 | **CONFIRMED, but NOT by this session's boots — carried, and dated 2026-08-15.** Both audit guests ran GNOME, whose mutter composites on vrend, so neither boot could reach venus present at all (the `limina-synoik-image` lesson: a mutter run cannot reproduce a venus-import bug, so its silence is a false negative). The evidence is the L2 test `synoik_session_reaches_a_rendered_desktop` (`crates/limina-test/tests/synoik_session.rs`), which EFI-boots the Vulkan-compositor image through exactly that path — **green in 28 s on 2026-08-15**. To re-measure it in a future pass, boot `Fedora-Workstation-44.enhanced.synoik.raw`, not an enhanced GNOME image. |
...
| 12a | Software-2D (`--gpu-software-2d`) is a usable llvmpipe desktop tier; on the enhanced image it shows only a blinking cursor "by design" | `docs/tiers.md` tier 1, `limina-tier2-venus` | **CONFIRMED and given a mechanism.** Booting `enhanced.raw` with `--gpu-software-2d`: `/dev/dri/card0` + `renderD128` exist and `drm_info` reports the `virtio_gpu` driver with a full mode list, but with no VIRGL capability gbm has no driver, so mutter logs `libEGL warning: egl: failed to create dri2 screen` → `Failed to create gbm device: No such file or directory` → `Failed to setup: No GPUs found`, `org.gnome.Shell@gdm.service: Failed with result 'protocol'`, and after several retries `GdmLocalDisplayFactory: maximum number of display failures reached. Giving up.` **Software-2D is a console/probe mode, not a GNOME desktop tier** — say so instead of calling it a degraded desktop. |

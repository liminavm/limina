#!/usr/bin/env bash
# What share of the host is left while a VM is banded?
#
#   host-throughput.sh <disk> <arm> [arm...]     arm = `none` (no VM) or a LIMINA_VCPU_SCHED value
#
# `host-impact.sh` measures the host's *latency*; this measures its *throughput*, and a reservation
# starves the two differently. A banded vCPU cannot be preempted, so ordinary host work does not
# lose punctuality to it — it loses share of the machine, which no deadline measurement reports.
#
# `hostwork` runs fixed work at ordinary priority and reports wall-clock: 1 thread asks whether a
# host process can get a core at all, 8 asks what fraction of the machine remains. Three reps,
# because everything in this spike has turned out noisier than it first looked.
set -uo pipefail
cd "$(dirname "$0")/../.."

disk=$1; shift
work=spikes/macos-timer-wakeup/hostwork
[ -x "$work" ] || clang -O2 -o "$work" spikes/macos-timer-wakeup/hostwork.c
...
bench() {          # bench <label>
  for th in 1 8; do
    for r in 1 2 3; do
      echo "$1 threads=$th rep=$r  $("$work" "$th" "$MEGA")"
    done
  done
}
...
for arm in "$@"; do
  ...
  LIMINA_DISK="$disk" LIMINA_NET=1 LIMINA_CPUS=8 RUST_LOG=limina=info,krun_vmm=info \
    LIMINA_VCPU_SCHED="$arm" \
    nohup spikes/venus-draw-probe/boot-enhanced-efi-kk.sh > "/tmp/limina-boot-$base.log" 2>&1 &
  boot=$!
  port=$(scripts/wait-guest-ssh.sh "$worker" 300 "$boot") || { echo "arm '$arm': no ssh"; continue; }
  ssh_guest() {
...
desaturate_guest() {
  ...
  echo "== policy='${arm:-none}'  armed vCPUs: $(grep -c 'VCPU-RT' "$worker" || echo 0)"
  sleep 60   # the desktop's first minute is login jobs, not idle

  bench "${arm:-unbanded} guest=idle"
  saturate_guest 180 || true
  bench "${arm:-unbanded} guest=sat "
  desaturate_guest

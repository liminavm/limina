...
Status: **proposed** (2026-08-02, user-decided pivot). Replaces the mechanism in
`display-modes.md` §"Fullscreen edge resistance", which stays documented as history. Companion:
`spikes/edge-pressure/RESULTS.md` (rounds 1–3, the measurements this rests on).

## Why the current design cannot be tuned into shape

`EdgeResist` lets the cursor escape and then warps it back. That is not an implementation detail
...
- **Losing key must ungrab immediately**, or a background VM holds a pointer it has no claim on.
  An earlier draft of this document said the existing capture path already handles this. **It does
...
## Order of work (revised after review)
...
chord. Anyone booting an intermediate commit (dogfood, a bisect) would hit a trapped pointer.

1. **Preset type + migration** (was 4): pure, unit-testable, no behavior change.
2. **The charge core and the re-grab hysteresis predicate**, as pure geometry in `fit.rs` with
   headless tests — the characterization-test pattern already there.
3. **Release mechanics**: warp-target computation (pure) plus the `end_warp_suppression` fix.
4. **Captured-scroll quality fix**, so step 5 cannot regress trackpads.
5. **Wire the policy** (was 1) — only once release, hysteresis, resign-key ungrab, the user latch
   and tap-gating all exist. One commit flips the felt behavior, atomically.
6. **Delete `EdgeResist`** — surgically. `edge_overflow`, `is_reflow` and the `emit_motion`
   pressure forwarding must survive: the hot corner for `Off` users depends on the last of them.
...
- Does `Firm` at 0.6 s feel like control or like a stuck pointer? Only dogfood answers it.
- Migration: existing `vm.toml` values are points (0/50/100/200). Map by preset position rather
  than by number, and treat an unrecognised number as `Standard`.
- Whether release should also require the press to be *outward* on a single axis, to keep a
  diagonal corner press from releasing sideways.

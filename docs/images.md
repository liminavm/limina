...
| `Fedora-Workstation-43.dev-enh.raw` | ⚠️ **STAGED FOR DELETION** (moved to `images-staging-delete/` 2026-06-25). The old **source-built** seated-venus golden (`/opt/mesa-zink`, x11-enabled venus ICD, apitrace + replay tooling, `limina-agent` + clipboard bridge, kmscube baked). Superseded for product/perf by `Fedora-Workstation-43.enhanced.raw`. One migration remains before it can be deleted — the `venus_replay` L2 test still defaults to it for its baked `apitrace`; see `images-staging-delete/README.md`. The `venus_replay` test SKIPs (disk-missing) until repointed. |
...
| `Fedora-Workstation-43.accessible.raw` | **Pristine-but-accessible base** (added 2026-06-25): a `…vanilla.raw` clone with gnome-initial-setup done (user `claude`), the host pubkey in `authorized_keys`, **autologin**, and **NOPASSWD sudo** (`/etc/sudoers.d/91-claude-nopasswd`), saved via a clean PSCI poweroff. The go-to start point for **enhanced-tier provisioning tests** and the **stock 4 KiB tier (software + virgl) perf control**: `cp -c` it, boot, scp the RPM payload, run `scripts/provision/install-enhanced.sh`. Stays stock until you opt in — no `/opt/mesa-zink` cruft (unlike the dev images), so the enhanced tier validates without the gdm crash-loop confound. mesa 25.2.4, kernel `6.17.1-300.fc43` (4 KiB). |
| `Fedora-Workstation-43.enhanced.raw` | **The canonical RPM-enhanced ("productized") image** (added 2026-06-25): a `…accessible.raw` clone with `scripts/provision/install-enhanced.sh` run to completion — the **16 KiB kernel** `6.12.0-limina16k+`, **mesa `26.2.0-1.limina`** (zink+venus, dnf-versionlocked), and the **patched mutter** all delivered as **RPMs replacing stock at `/usr`** (the [[limina-enh-delivery]] mechanism). Rebooted into 16k, GNOME came up seated & stable, **venus desktop pixel-verified** (zink→venus→KosmicKrisp→Metal, scanout IOSurface 99.99% nonzero). This is the **RPM-delivery counterpart** to the source-built `dev-enh.raw`, and the image to use for **enhanced-tier (venus) perf + validation** going forward. On-display glmark2 (build+shading+texture) = **2784**. Was `/tmp/enh-test.raw` during creation; promoted to a permanent home 2026-06-25. |

Boot the enhanced tier: `scripts/run-enhanced.sh [--window | --capture <png>]` (clones internally), or for
the seated-venus perf flow reuse the validated image directly without re-cloning:

...
| 0093 vsock timesync: saturate elapsed-time arithmetic against backward clock steps | debug-build panic fix | `now - last_awake` on raw CLOCK_REALTIME reads underflows on an NTP/manual backward step (panic in debug, huge wrap in release); `saturating_sub` + unit tests. Upstream libkrun has the identical expression. |
| 0094 hvf: model PSCI CPU_OFF + AFFINITY_INFO and make CPU_ON re-deliverable at runtime (guest vCPU hotplug) | correctness/robustness | ✚ **send** — a guest offlining a vCPU (`echo 0 > .../cpuN/online`) wedges the VMM today: CPU_OFF/AFFINITY_INFO return NOT_SUPPORTED → the dying CPU busy-spins in `cpu_die` (~546% host CPU) and the offline never completes; re-online was impossible (one-shot boot channel). Upstream libkrun (macOS hvf backend) has the same gap. CPU_OFF now parks the vCPU thread cleanly; AFFINITY_INFO reports ON/OFF; CPU_ON re-arms the parked thread (SCTLR MMU-off reset). Mechanism only, no policy. spikes/wakeup-probe/vcpu-offline-probe.sh, tests/l2_vcpu_hotplug.rs |
...
| patch | one-line | note |
|---|---|---|
| 0041 (+**0043**) vkr: ring relax backoff — adaptive plateau, not per iteration | perf; replaces upstream's literal `/* TODO do better */` | upstream ladder = ~55 timed wakeups per 1 ms ring idle window; fix = a two-phase backoff (40 µs responsive plateau for a fed ring → 640 µs deep-idle). **Squash 0043 into 0041 before sending** — 0043 supersedes 0041's flat quad-rung cap after an A/B showed the 640 µs cap halved uncapped vkmark throughput (1193 vs ~2360) with no idle-wakeup benefit over the plateau. OS-agnostic, applies verbatim. spikes/wakeup-probe/RESULTS.md |

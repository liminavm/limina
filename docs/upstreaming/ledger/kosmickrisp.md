...
- **Dropped, upstream implementation adopted:** 0005 custom-border (`4c8e720c`; the worker
  ...
  the whole timestamp arc 0008+0010–0013 (`ed807097` + `7f540b05`; the M4 Pro
  probe-vs-upstream action item below STANDS), 0016's core hunks (`479773c7e42`), the
  monolith's vn_wsi deep-copy and dm-assert components (`311a437c`, `bbf67327`).
- **Removed pending re-benchmark (not dropped on the merits):** the 0017+0018
  threaded-submit pair — live-record deleted its premise (encoding now happens at record
  time); revive from the tag only if a tip benchmark shows a remaining submit tax.

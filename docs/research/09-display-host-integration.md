...
- **C. Zero-copy IOSurface (v2, needs libkrun patch + doc 03).** The current vtable cannot do this — would require **patching `libkrun_display.h`** to add a new `features` bit + callback that hands the backend a host-mapped blob / IOSurface / fd for the scanout (so virgl/MoltenVK renders straight into an IOSurface-backed `MTLTexture`). Highest value for smooth 3D, highest cost; defer.

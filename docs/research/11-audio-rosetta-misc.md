...
| Fact | Evidence |
|------|----------|
| ~~`snd` Cargo feature exists as an **empty stub**~~ — **stale (re-verified 2026-07-17): the stub feature is GONE.** `grep snd third_party/libkrun/src/libkrun/Cargo.toml third_party/libkrun/src/vmm/Cargo.toml` finds nothing; there is no `snd` feature to "fill" anymore. | (was `src/libkrun/Cargo.toml:18` / `src/vmm/Cargo.toml:22` in an older tree) — the native-device work must **add** the feature fresh, not fill an existing one. |
...
**Decisive rationale — the two-tier guarantee (this is the load-bearing reason, holds even if the effort doubles):** stock Fedora ships the `snd-virtio` driver (`CONFIG_SND_VIRTIO`), so a native in-VMM virtio-snd device gives an **unmodified, stock guest sound with zero guest-side limina components**. Every alternative that keeps audio in the guest and streams it out (A3, vsock→host player) needs a guest agent → a stock guest gets **silence** → it **violates limina's hard two-tier constraint** (stock baseline must be usable). A2 is the only option that satisfies it. Record this as *the* reason to build the native device.

Secondary rationale (corrected from the first draft): libkrun's generic vhost-user path is **Linux-host-only** — `attach_vhost_user_device`, the RNG fallback, and the file-backed (memfd) guest memory a backend needs to mmap are all `#[cfg(all(feature = "vhost-user", target_os = "linux"))]` (`src/vmm/src/builder.rs:976,:991,:1378-1381,:1635-1638,:2454-2455`). The header pre-defining `KRUN_VHOST_USER_DEVICE_SND` (`include/libkrun.h:772-779`) is only the C surface; there is no macOS backing — and beyond memfd, vhost-user passes kick/call **eventfds** over SCM_RIGHTS, which macOS lacks (a third porting axis). So "zero libkrun patching via vhost-user" is **false on macOS**. The native device runs inside the VMM with direct guest-memory access, sidestepping the memfd/eventfd problems entirely.

The device is modeled on the reserved `KRUN_VIRTIO_DEVICE_SND 25` id — but see the two structural notes below, which override the naive "model on rng/console" framing.

**Structural decision 1 — split the model: queue plumbing like `rng`, threading like `gpu`.** The `rng`/`i2c` devices service their virtqueues *inline on the shared `EventManager` epoll thread* that also runs net/block/console/fs. Audio serviced there **will glitch** whenever a disk flush or GL burst hogs that loop. The snd device needs the **`gpu` device's threading**: a dedicated worker thread (cf. `gpu/worker.rs`) plus a **lock-free SPSC ring** to the CoreAudio render callback, which runs on Apple's realtime HAL thread and may take **no locks, no allocations, no unbounded msgSend**. Copy `rng` for how a native macOS device is wired into the builder; do **not** copy its thread model.

**Structural decision 2 — buffer-completion pacing is the make-or-break (name it explicitly).** The guest ALSA driver advances its `hw_ptr` on **tx-queue buffer completions**. Every other in-tree device completes descriptors as soon as it copies them out — do that here and the guest playback clock free-runs ahead of the DAC → continuous xruns. tx descriptors must be completed **only after the CoreAudio render callback has consumed those frames**. Done right, the guest audio clock *becomes* the host DAC clock by construction — no drift compensation, and the "clock-sync hell" that kills A3 never arises for A2. This is the single likeliest multi-day sink; the go/no-go spike must verify it directly (sustained xrun-free playback, incl. under GL+disk load).

What must be built/patched for audio:
- **Native virtio-snd device in libkrun** under `src/devices/src/virtio/snd/` (control/event/tx/rx queues per `include/libkrun.h:776-779`), wired into the builder for macOS. **Add** the `snd` feature + a `krun_add_snd`-style C API + builder wiring (the old empty `snd` Cargo stub is **gone** from this tree — nothing to "fill"). Do **not** hand-write the virtio-snd protocol state machine from scratch: adapt crosvm's `virtio-snd` or rust-vmm `vhost-device-sound` (both permissively licensed — BSD-3 / Apache-2.0) for the control-plane surface (PCM_INFO/SET_PARAMS/PREPARE/START/STOP/RELEASE); QEMU `audio/coreaudio.m` is the reference for the CoreAudio pacing *behavior* (GPL — read, don't copy).
- A **CoreAudio sink/source** — target the C **AudioUnit v2 `kAudioUnitSubType_DefaultOutput` (AUHAL)** for output: not deprecated on macOS 26, does format conversion internally, and **follows the host default-device switch automatically** (deletes the hot-switch work item). Input/mic second, incl. macOS TCC mic-permission handling — note the prompt originates in the **child `limina-vmm` process**, not the AppKit app, so `NSMicrophoneUsageDescription` must be in *both* Info.plists and the supervisor should request authorization up front (cf. the Accessibility-TCC-per-rebuild saga).
- **Advertise the minimum**: one playback stream, 48 kHz / S16_LE stereo, jacks=0, one FL/FR chmap. PipeWire remixes/resamples in the guest and AUHAL converts on the host, so host-side sample-rate conversion and multiple streams are v1 scope creep.
- Guest: `CONFIG_SND_VIRTIO` (**note the symbol — it is `SND_VIRTIO`, not `VIRTIO_SND`**). Stock Fedora has it as `=m`, and the **enhanced 16k kernel inherits it** (its RPM is built from the full Fedora config; verified §5.9) — so **both tiers already have the driver; no kernel/config change is needed for audio.**
- **M9 suspend/resume interaction** (just shipped): the device must serialize stream state (params, prepared/started, pending descriptors) and rebuild the AudioUnit on restore; define + test v1 behavior (e.g. "resume drops to a benign xrun, guest PipeWire recovers").
- Fallback if native proves too costly: A1 (port vhost-user to macOS + CoreAudio backend for `vhost-device-sound`) — larger scope (memfd + eventfd + a new backend), only if we want process isolation (marginal here — the GPU stack already runs in-process in the same worker).
...
1. **Native virtio-snd device spike (go/no-go)** — stand up a minimal `snd/` device (queue plumbing like `rng`, threading like `gpu`, per §4), advertise one 48 kHz/S16/stereo playback stream, enumerate the card in the guest, and play a tone. Measure, in priority order: **(a) pacing correctness** — 10+ min sustained playback with zero guest xruns (`cat /proc/asound/card*/pcm0p/sub0/status`) and no cumulative `hw_ptr`-vs-wallclock drift (this is what falsifies a wrong completion-pacing model, the top risk); **(b) contention** — repeat (a) *while* a GL workload + disk I/O run (falsifies the shared-epoll-thread temptation); **(c) idle cost** — stream open but silent: CPU/wakeups (limina has eaten idle-wakeup regressions before, cf. the venus-ring saga); **(d) latency** — round-trip vs. Parallels. This is the make-or-break for A2.
...
7. ~~**Native `snd`/`rtc` feature reality**~~ — **RESOLVED 2026-07-17:** there is no half-wired snd path. No `snd/` device dir, no `snd` Cargo feature (the stub was removed), no `krun_set_snd_device`/`krun_add_snd` symbol — only the header id/queue constants. Clean slate confirmed. (`rtc` similarly has no native device; still vhost-user-only i.e. Linux-only.)
...
9. ~~**Does the enhanced 16k kernel carry `CONFIG_SND_VIRTIO`?**~~ **RESOLVED 2026-07-17 — NO GAP, no config change needed.** A review draft claimed the enhanced kernel lacks any `SND` config and would ship silent, citing `scripts/build-test-kernel.sh`. That is the **wrong script**: `build-test-kernel.sh` builds the L1 **test-inject** kernel (arm64 `defconfig` + a small fragment → `Image`/`Image-16k`, consumed via the FRINGE direct-`--kernel` path), *not* the enhanced-image kernel. The **actual enhanced-tier kernel** (`scripts/provision/f44/build-kernel-rpm.sh`) is built **from the running guest's full Fedora config** (`.config = /boot/config-$(uname -r)`, then `merge_config.sh -m` layers only a 16k-page + build-hygiene fragment on top; lines 4-5, 30, 68, 103). It therefore **inherits Fedora's `CONFIG_SND_VIRTIO=m`** untouched — virtio-snd ships as a runtime-loadable module, exactly like stock. Corroboration: the adjacent libkrunfw aarch64 config has the full stack (`CONFIG_SND_VIRTIO=y`, `CONFIG_SND=y`, `CONFIG_SND_PCM=y` — `config-libkrunfw_aarch64:2114-2147`). Optional airtight confirmation left undone: boot `Fedora-Workstation-44.enhanced.test.raw` and read `/boot/config-*` / `modinfo virtio_snd`.
...
- `third_party/libkrun/src/libkrun/Cargo.toml:23` (`rosetta`) — empty stub feature. **NOTE:** the `snd` stub features (once at `libkrun/Cargo.toml:18` / `vmm/Cargo.toml:22`) are **no longer in the tree** (re-verified 2026-07-17) — the native-device work adds `snd` fresh.

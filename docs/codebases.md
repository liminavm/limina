...
| **Guest kernel** (16k) | `kernel.org` `linux-stable.git`, branch = the tag (e.g. `v7.1.5`) | `7.1.5-limina16k` deployed (dogfood); `7.1.4-limina16k` in the enhanced images | `patches/linux/` 0001–0006 (virtio-gpu scanout/modifier, **0004 16 KiB host-visible alignment**, **0005 balloon page-reporting across suspend**, **0006 XBGR/ABGR primary plane**) | `scripts/provision/f44/build-kernel-rpm.sh` on a dev-Mac F44 build guest |

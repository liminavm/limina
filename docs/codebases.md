...
- **GUEST binaries** run inside the Linux guest, i.e. baked into the `*.raw` images. This is
  the guest kernel, the guest Mesa (venus + zink), `limina-agent` and
  `limina-agent-session` (guest mutter is STOCK since 2026-07-11, and the
  `clipboard@limina` gnome-shell extension was retired 2026-08-15 — the GNOME clipboard is
  stock `spice-vdagent` now).
...
| `third_party/virglrs/third_party/virglrenderer` | **fork model** `liminavm/virglrenderer` branch `limina`, pinned by **virglrs**, not by limina | **The C reference the Rust rewrite generates format tables from and records goldens against** — not what the worker runs, and **not limina's to pin**: there is no `[virglrenderer]` in `third_party/manifest.toml`, because one owner is enough and this is not it. Both accelerated tiers: venus (Vulkan→KK) and vrend (GL via zink). Our macOS/venus enablement, IOSurface scanout, vkr fixes; major later additions: snapshot/restore journal (0033–0040), ring-wake profiling (0046–0048), ring-fatal fix (0058). This is also where upstreaming work happens (`docs/upstreaming/ledger/virglrenderer.md`). | **none** — the branch IS the delta; `patches/virglrenderer/` retired 2026-08-04 | materialized by virglrs's own `scripts/vendor.sh`; a spike that needs `config.h` must `meson setup build` there itself |
| `third_party/virgl-gl-prefix/` | — | Stale: a secondary C virglrenderer build output. Nothing in limina builds or loads the C renderer any more. | — | prefix only |
| `third_party/mesa-cs.sparseimage` → **`/Volumes/mesa-cs`** | **fork model** `liminavm/mesa` branch `limina-kk`, pinned in `third_party/manifest.toml`; **26.2.0-devel** | **HOST Mesa.** Builds **KosmicKrisp** (`libvulkan_kosmickrisp.dylib`, the sole venus backend) and **zink-on-KK** (host GL). `-Dplatforms=macos`. **venus is NOT built here** (`build-kk/src/virtio/vulkan/` is empty — the venus source is present-but-inert). Build dirs: `build-kk`, `build-zink-kk`, prefix `zink-kk-prefix`. **Both build dirs must be configured `-Db_ndebug=true`** (keep `buildtype=debugoptimized`, so `-O2`/`-g` and crash-report symbolication are unaffected): with asserts compiled in, a guest trips one — invalid Vulkan usage on the vkr ring thread, or a GL stream on zink's threaded-context worker — and SIGABRTs the worker, killing the VM. `scripts/build-app.sh` refuses to bundle **any** Mesa dylib that still references `__assert_rtn`, because this is meson state on the sparse image that nothing in the repo pins. | **none** — the branch IS the delta (20 commits over base `178a3d739`); `patches/kosmickrisp/` retired 2026-08-04. NOT vendored by `cargo xtask vendor` (case-sensitive sparse image) | **`scripts/build-host-mesa.sh`** (`cargo xtask mesa`) — creates the sparse image, clones the fork at its pin and builds BOTH dirs with `-Db_ndebug=true`. Toolchain is version-coupled: `mesa_clc` compiles `src/kosmickrisp/libkk/*.cl` against libclc's `spirv64-mesa3d-.spv`, which only **libclc 22.x** provides (23.x dropped the `.pc` Mesa finds it by and renamed the payload), so `[libclc]` in `third_party/manifest.toml` pins the bottle by digest and the script fetches it into `third_party/libclc/`. → `libvulkan_kosmickrisp.dylib`, zink `libEGL/libGL` |
...
|---|---|---|---|---|
| **Guest Mesa** (venus; guest GL is virgl/vrend since drop-guest-zink 2026-08-04) | **fork**: `github.com/liminavm/mesa`, branch `limina-guest`, base `mesa-26.1.5` (the Fedora SRPM base both tracks build); pinned in `third_party/manifest.toml [mesa-guest]`; worktree `/Volumes/mesa-cs/mesa-guest` | `mesa 26.1.5-6.limina.fc44` (LIMINA_REL=7 respin incoming, task #11) | 6 venus commits on the branch, exported by `scripts/export-mesa-guest-patches.sh` into the committed `patches/mesa-guest/` series (the old `patches/mesa/` pool is a tombstone) | `scripts/provision/f44/build-mesa-rpm.sh` — in the unified F44 build container (`cargo xtask enhanced mesa`) or inside a booted F44 guest, same script; applies the whole series via the spec so a non-applying patch fails %prep loudly; dnf-versionlocked |
| **Guest kernel** (16k) | **fork**: `github.com/liminavm/linux` (of `gregkh/linux`, the stable mirror), branch `limina`, base `v7.1.8`; pinned in `third_party/manifest.toml` | `7.1.8-limina16k` in the F44 enhanced images | **Two commits over `v7.1.8`** (`6292465948e0` expose per-scanout rects as suggested connector offsets, `89ba418db60b` type and attach PRIME-imported dmabufs) — `scripts/export-linux-patches.sh` derives them for the consumers that apply a series. The pre-7.1 series is gone: the modifier/primary-plane work was **deliberately dropped** when we moved to upstream's direction of *not advertising fb modifiers* on virtio-gpu (synoik was patched to accept INVALID-modifier planes instead, #39); the page-reporting-vs-suspend backport landed upstream (`0b45f69`). The kernel is still a fork so we can carry a patch the moment we need one — it just carries none. *(16 KiB host-visible alignment left the series 2026-08-03 — a no-op on the 16k kernel; it lives in `guest/virtio-gpu-dkms/` for the stock-4k tier.)* Consequence to remember: **`drmModeAddFB2WithModifiers` returns EINVAL** on this kernel (`drm_internal_framebuffer_create: driver does not support fb modifiers`), so anything that scans out must fall back to plain `drmModeAddFB2`. | `scripts/provision/f44/build-kernel-rpm.sh` — in the unified F44 build container (`cargo xtask enhanced kernel`) or inside a booted F44 guest (builds the pinned rev; no patch stage). The base config differs by where it runs: the running kernel's in a guest, the image's `kernel-core` in the container (`CONFIG_BASE` overrides). |
| **Guest mutter** | **STOCK Fedora** (since 2026-07-11 the payload ships NO mutter; the GNOME clipboard is stock `spice-vdagent`) | distro's own (e.g. `50.3-2.fc44`) | `patches/mutter/` 0003 kept UNSHIPPED for ext-data-control experiments (0001/0002 retired) | optional: `scripts/provision/f44/build-mutter-rpm.sh` |
| ~~**clipboard@limina** (gnome-shell extension)~~ | RETIRED 2026-08-15 (#37 step 4) — `guest/gnome-shell-extension/` is gone and `install-enhanced.sh` REMOVES it from guests that carry it | — | — | GNOME's clipboard is stock `spice-vdagent`; `limina-agent-session` yields to it |

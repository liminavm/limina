...
|---|---|---|---|---|
| `third_party/libkrun` | `limina/macos-venus` family | The VMM (HVF + virtio + rutabaga). Our devices/APIs. | `patches/libkrun/` (UPSTREAM_BASE `07a3f40`) | linked by `limina-vmm` |
| `third_party/libkrunfw` | — | Bundled guest kernel (`linux-6.12.x`) for the **non-EFI** direct-boot path only. | — | libkrunfw dylib |
| `third_party/virglrenderer` | `limina/macos-venus`, head `a155155` | **THE host GPU renderer.** Both accelerated tiers: venus (Vulkan→KK) and vrend (GL via zink). Our macOS/venus enablement, IOSurface scanout, vkr fixes. | `patches/virglrenderer/` 0001–0025 (UPSTREAM_BASE `2048dfb`) | `third_party/virgl-prefix/` ← **the worker links this** (verify: `otool -L target/debug/limina-vmm \| grep virgl`) |
...
### Our own Rust crates (`crates/`) — host, except the agent
`limina` (app/CLI/supervisor) · `limina-vmm` (the VMM worker child) · `limina-display` ·
`limina-input` · `limina-proto` (control-plane wire types) · `limina-surfaceport` ·
`limina-usbip` · `limina-test` (L0/L1/L2 harness). **`limina-agent` is the one guest-side
binary we own** (built for `aarch64-unknown-linux-musl`/gnu, delivered into the guest).
...
1. **`mesa-cs` is the HOST Mesa (KosmicKrisp + zink-on-KK), not the guest venus driver.**
   ...
   than the guest ships (26.1.3). To change guest venus, work against the **F44 mesa SRPM
   version + `patches/mesa/` 0009–0013**, not `/Volumes/mesa-cs`. venus only ever runs in the
   guest; KosmicKrisp only ever runs on the host.
2. **`patches/mesa/` serves two different builds.** `build-mesa-rpm.sh` applies **0001 + 0009–0013**
   to the **guest** RPM. The **host** KK/zink build (`mesa-cs`, branch `limina/kosmickrisp`) carries
